...
# 主题或附件名带「发票」时优先保留；其它 PDF 仍尝试抽金额
...
def download_invoices(
    ...
    try:
        ...
        for num in numbers:
            ...
            _ingest_message(message, pool, unknown, ledger, stats)
...
def _ingest_message(
    ...
    if message_id and message_id in ledger.message_ids:
    ...
    prefer = bool(INVOICE_HINT_RE.search(subject)) or any(
        INVOICE_HINT_RE.search(name) for name, _ in attachments
    )
    saved_any = False
    ...
    # 整封处理完再记 Message-ID，避免中途失败导致漏文件
    if message_id and (saved_any or attachments):
        ledger.message_ids.add(message_id)
